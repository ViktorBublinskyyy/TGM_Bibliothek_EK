...
class LibraryDatabase {
  ...
  async getFavoriteBooks(userId) {
    try {
      ...
      const books = await mainCollection.find({ b_id: { $in: bookIds } }).toArray();
      ...
      booksWithUserId.forEach(book => {
        console.log(book);
      });
      return booksWithUserId;
    } catch (err) {
      console.error(err);
    }
  }
...
dbClient.connect()
  .then(() => dbClient.getFavoriteBooks(1)) // replace "yourUserId" with an actual user ID
  .then(() => dbClient.close())
  .catch(console.dir);
